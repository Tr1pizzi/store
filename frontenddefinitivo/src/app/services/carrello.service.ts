...
export class CarrelloService {
  ...
  aggiungiArticolo(ID: number, quantita: number) {
    this.http.post<Carrello>(this.URL + "/aggiungi/" + this.carrello.getValue()?.id + "/" + ID + "/" + quantita, {}, this.httpOptions).subscribe({
      next: data => {
        this.sendUpdate(data);
      },
      error: err => {
        console.log("errore aggiunta prodotto");
      }
    });
  }

  rimuoviArticolo(libroID: number, quantita: number) {
